...
const TikTokLinkFeature: FC<{ feature: IFeature }> = ({ feature }) => {
  ...
  return (
    <div className="flex flex-grow px-[24px]">
      <a
        target="_blank"
        href={feature.values.tikTokValues?.link}
        style={{
      ...
      >
        {feature.values.tikTokValues?.image ? (
          <figure className="h-8 w-8 overflow-hidden rounded-full">
            <LazyLoadImage
              src={getPublicImageURL(feature.values.tikTokValues?.image)}
              alt="link-img"
        ...
        <div className="font-[400] text-md truncate">
          {feature.values?.tikTokValues?.title}
        </div>
        <figure className="h-8 w-8">
...
export default TikTokLinkFeature;
