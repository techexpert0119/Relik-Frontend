...
const XlinkFeature: FC<{ feature: IFeature }> = ({ feature }) => {
  ...
  return (
    <div className="flex flex-grow px-[24px]">
      <a
        href={feature.values.xLinkValues?.link}
        target="_blank"
      ...
      >
        {feature.values.xLinkValues?.image ? (
          <figure className="h-8 w-8 overflow-hidden rounded-full">
            <LazyLoadImage
              src={getPublicImageURL(feature.values.xLinkValues?.image)}
              alt="link-img"
              className="h-full w-full object-cover"
            />
          </figure>
        ) : (
          <figure className="h-5 w-8">
            <XLogo color={page?.theme.buttonFontColor} height={20} width={32} />
          </figure>
        )}
        <div className="font-[400] text-md truncate">
          {feature.values.xLinkValues?.title}
        </div>
