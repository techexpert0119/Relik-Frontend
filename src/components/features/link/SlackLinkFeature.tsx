...
const SlackLinkFeature: FC<{ feature: IFeature }> = ({ feature }) => {
  ...
  return (
    <div className="flex flex-grow px-[24px]">
      <a
        target="_blank"
        href={feature.values.slackValues?.link}
        style={{
      ...
      >
        {feature.values.slackValues?.image ? (
          <figure className="h-8 w-8 overflow-hidden rounded-full">
            <LazyLoadImage
              src={getPublicImageURL(feature.values.slackValues?.image)}
              alt="link-img"
              className="h-full w-full object-cover"
            />
          </figure>
        ) : (
          <figure className="h-[34px] w-8">
            <SlackLogo
              color={page?.theme.buttonFontColor}
              height={34}
              width={32}
        ...
        <div className="font-[400] text-md truncate">
          {feature.values?.slackValues?.title}
        </div>
...
export default SlackLinkFeature;
