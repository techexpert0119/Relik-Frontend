...
const TwitchLinkFeature: FC<{ feature: IFeature }> = ({ feature }) => {
  ...
  return (
    <div className="flex flex-grow px-[24px]">
      <a
        target="_blank"
        href={feature.values.twitchValues?.link}
        style={{
      ...
      >
        {feature.values.twitchValues?.image ? (
          <figure className="h-8 w-8 overflow-hidden rounded-full">
            <LazyLoadImage
              src={getPublicImageURL(feature.values.twitchValues?.image)}
              alt="link-img"
        ...
        <div className="font-[400] text-md truncate">
          {feature.values?.twitchValues?.title}
        </div>
...
export default TwitchLinkFeature;
