...
const LinkFeature: FC<{ feature: IFeature }> = ({ feature }) => {
  ...
  return (
    <div className="flex flex-grow px-[24px]">
      <a
        href={feature.values.linkValues?.link}
        target="_blank"
      ...
      >
        {feature.values.linkValues?.image ? (
          <figure className="h-8 w-8 overflow-hidden rounded-full">
            <LazyLoadImage
              src={getPublicImageURL(feature.values.linkValues?.image)}
              alt="link-img"
          ...
          <figure className="h-6 w-8 ">
            <Link className="h-6 w-8" />
          </figure>
        )}
        <div className="font-[400] text-md truncate">
          {feature.values.linkValues?.title}
        </div>
