...
const TelegramLinkFeature: FC<{ feature: IFeature }> = ({ feature }) => {
  ...
  return (
    <div className="flex flex-grow px-[24px]">
      <a
        target="_blank"
        href={feature.values.telegramValues?.link}
        style={{
      ...
      >
        {feature.values.telegramValues?.image ? (
          <figure className="h-8 w-8 overflow-hidden rounded-full">
            <LazyLoadImage
              src={getPublicImageURL(feature.values.telegramValues?.image)}
              alt="link-img"
              className="h-full w-full object-cover"
            />
          </figure>
        ) : (
          <figure className="h-[30px] w-8">
            <TelegramLogo
              color={page?.theme.buttonFontColor}
              height={30}
              width={32}
        ...
        <div className="font-[400] text-md truncate">
          {feature.values?.telegramValues?.title}
        </div>
...
export default TelegramLinkFeature;
