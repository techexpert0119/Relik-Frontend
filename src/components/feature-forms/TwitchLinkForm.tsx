...
const TwitchLinkForm = () => {
  ...
  useEffect(() => {
    if (options?.featureId) {
      ...
      if (feature) {
        form.reset({
          link: feature.values.twitchValues?.link,
          title: feature.values.twitchValues?.title,
        });
      }
    }
  ...
  const onSubmit = (values: z.infer<typeof schema>) => {
    if (!options?.featureId) {
      createFeature?.({
        type: PageFeatureType.JOIN_TO_TWITCH,
        values: {
          twitchValues: {
            link: values.link,
            title: values.title,
