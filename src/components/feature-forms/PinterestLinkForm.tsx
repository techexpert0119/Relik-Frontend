...
const PinterestLinkForm = () => {
  ...
  useEffect(() => {
    if (options?.featureId) {
      ...
      if (feature) {
        form.reset({
          link: feature.values.pinterestValues?.link,
          title: feature.values.pinterestValues?.title,
        });
      }
    }
  ...
  const onSubmit = (values: z.infer<typeof schema>) => {
    if (!options?.featureId) {
      createFeature?.({
        type: PageFeatureType.PINTEREST,
        values: {
          pinterestValues: {
            link: values.link,
            title: values.title,
...
export default PinterestLinkForm;
