...
const HuaweiStoreLinkForm = () => {
  ...
  useEffect(() => {
    if (options?.featureId) {
      ...
      if (feature) {
        form.reset({
          link: feature.values.huaweiStoreLinkValues?.link,
          variant: feature.values.huaweiStoreLinkValues?.variant,
        });
      }
    }
  ...
  const onSubmit = (values: z.infer<typeof formSchema>) => {
    if (!options?.featureId) {
      return createFeature?.({
        type: PageFeatureType.HUAWEI_APP,
        values: {
          huaweiStoreLinkValues: { link: values.link, variant: values.variant },
        },
      });
    } else {
      return updateFeature?.(options?.featureId, {
        ...
        form.reset({
          link: res.values.huaweiStoreLinkValues?.link,
          variant: res.values.huaweiStoreLinkValues?.variant,
        });
      });
    }
  };
...
export default HuaweiStoreLinkForm;
