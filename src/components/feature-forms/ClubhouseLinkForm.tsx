...
export const schema = z.object({
  ...
  link: z
    ...
    .min(4, { message: 'Link must be at least 4 characters long!' })
    .url({ message: 'Please enter a valid URL!' }),
});

const ClubhouseLinkForm = () => {
  ...
  useEffect(() => {
    if (options?.featureId) {
      ...
      if (feature) {
        form.reset({
          link: feature.values.clubhouseValues?.link,
          title: feature.values.clubhouseValues?.title,
        });
      }
    }
  ...
  const onSubmit = (values: z.infer<typeof schema>) => {
    if (!options?.featureId) {
      createFeature?.({
        type: PageFeatureType.JOIN_TO_CLUBHOUSE,
        values: {
          clubhouseValues: {
            link: values.link,
            title: values.title,
...
export default ClubhouseLinkForm;
